...
form.addEventListener('submit', async (event) => {
  ...
  // Remove campos que pertencem a DanioRerio_BOX
  delete baseData.box;
  ...
  if (sampleToEdit) {
    ...
    ({ error: errorSeabass } = await supabase.from('DanioRerio').insert([baseData]));

    ({ error: errorBox } = await supabase.from('DanioRerio_BOX').insert([
  ...
  if (errorSeabass || errorBox) {
    alert('Erro ao salvar:\n' +
      ...
      (errorBox?.message || ''));
    console.error('Erro DanioRerio:', errorSeabass);
    console.error('Erro DanioRerio_BOX:', errorBox);
  } else {
    alert(sampleToEdit ? 'Amostra editada com sucesso!' : 'Amostra adicionada com sucesso!');
    form.reset();
    dynamicSectionsContainer.innerHTML = '';
    window.location.href = '../danio-index.html';
  }
});

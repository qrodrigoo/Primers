...
async function testSupabaseConnection() {
    ...
    try {
        const { data, error } = await supabase
            .from('DanioRerio') // Tente buscar de uma de suas tabelas principais
...
async function fetchBoxLocations() {
    ...
    try {
        ...
        console.log("fetchBoxLocations: Dados brutos de DanioRerio_BOX recebidos:", data);
...
async function fetchSeabassData() {
    console.log("fetchSeabassData: Tentando buscar todos os dados de DanioRerio...");
    try {
        const { data, error } = await supabase
            .from('DanioRerio')
            .select(`
        ...
        if (error) {
            console.error("fetchSeabassData: ERRO ao buscar dados de DanioRerio:", error.message);
            return [];
        }

        allSeabassSamples = data;
        console.log("fetchSeabassData: Dados de DanioRerio buscados com sucesso:", allSeabassSamples);
        return data;
    } catch (error) {
        console.error("fetchSeabassData: ERRO inesperado ao buscar dados de DanioRerio:", error);
        return [];
    }
}
...
document.addEventListener('DOMContentLoaded', async () => {
    console.log("DOMContentLoaded: Página DanioRerio carregada. Iniciando setup...");
    ...
    console.log("DOMContentLoaded: Página DanioRerio inicializada.");
    ...
    // 1. Deleta da tabela DanioRerio
    const { error: errorSeabass } = await supabase
        .from('DanioRerio')
        .delete()
        .eq('abbr', sample.abbr);

    // 2. Deleta da tabela DanioRerio_BOX
    ...
    if (errorSeabass || errorBox) {
        ...
        console.error("Erro ao excluir da DanioRerio:", errorSeabass);
        console.error("Erro ao excluir da DanioRerio_BOX:", errorBox);
        return;
    }
...
document.getElementById('editBoxCountBtn').addEventListener('click', async () => {
  ...
  const { data, error } = await supabase
    .from('config')
    .select('num_boxes')
    .eq('page', 'DanioRerio') // ajuste o nome da página conforme sua base
    .single();
...
async function fetchBoxCount() {
  try {
    const { data, error } = await supabase
      .from('config')
      .select('num_boxes')
      .eq('page', 'DanioRerio') // nome da página
      .single();

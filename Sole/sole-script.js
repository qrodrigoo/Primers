...
function initializeLocationBoxes() {
    ...
    console.log(`initializeLocationBoxes: ${boxCount} caixas de localização criadas no DOM.`); // Corrigido para 2 caixas
}
...
document.getElementById('editBoxCountBtn').addEventListener('click', async () => {
  ...
  const { data, error } = await supabase
    .from('config')
    .select('num_boxes')
    .eq('page', 'Sole') // ajuste o nome da página conforme sua base
    .single();

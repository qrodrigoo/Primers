...
form.addEventListener('submit', async (event) => {
  ...
  if (sampleToEdit) {
    ...
    ({ error: errorSeabass } = await supabase.from('Turbot').insert([baseData]));

    ({ error: errorBox } = await supabase.from('Turbot_BOX').insert([
  ...
  if (errorSeabass || errorBox) {
    alert('Erro ao salvar:\n' +
      ...
      (errorBox?.message || ''));
    console.error('Erro Turbot:', errorSeabass);
    console.error('Erro Turbot_BOX:', errorBox);
  } else {
    alert(sampleToEdit ? 'Amostra editada com sucesso!' : 'Amostra adicionada com sucesso!');
    form.reset();
    dynamicSectionsContainer.innerHTML = '';
    window.location.href = '../turbot-index.html';
  }
});

...
form.addEventListener('submit', async (event) => {
    ...
    const tilapiaData = {
        ...
        "biorad.annealing_temperature": formData.get('biorad.annealing_temperature'),
        "biorad.slope": formData.get('biorad.slope'),
        "biorad.efficiency": formData.get('biorad.efficiency'),
        "biorad.efficiency.porcentagem": formData.get('biorad.efficiency.porcentagem'),
        "biorad.obervations": formData.get('biorad.obervations')
    };
    ...
    if (sampleToEdit) {
        ...
        console.log('Resultado Update LVShrimp:', errorTilapia); // Log 5
        ...
        console.log('Resultado Update LVShrimp_BOX:', errorTilapiaBox); // Log 6

        localStorage.removeItem('sampleToEdit');
    } else {
        // MODO ADIÇÃO
        ({ error: errorTilapia } = await supabase.from('LVShrimp').insert([tilapiaData]));
        console.log('Resultado Insert LVShrimp:', errorTilapia); // Log 7

        ({ error: errorTilapiaBox } = await supabase.from('LVShrimp_BOX').insert([tilapiaBoxData]));
        console.log('Resultado Insert LVShrimp_BOX:', errorTilapiaBox); // Log 8
    }
    ...
    if (errorTilapia || errorTilapiaBox) {
        ...
        console.error('Erro LVShrimp pia Final:', errorTilapia);
        console.error('Erro LVShrimp_BOX Final:', errorTilapiaBox);
    } else {
        alert(sampleToEdit ? 'Amostra editada com sucesso!' : 'Amostra adicionada com sucesso!');
        form.reset();
        window.location.href = '../lvshrimp-index.html';
    }
});

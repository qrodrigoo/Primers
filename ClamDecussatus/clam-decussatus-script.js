...
async function testSupabaseConnection() {
    ...
    try {
        const { data, error } = await supabase
            .from('Clam_Decussatus') // Tente buscar de uma de suas tabelas principais
...
async function fetchSeabassData() {
    console.log("fetchSeabassData: Tentando buscar todos os dados de Clam_Decussatus...");
    try {
        const { data, error } = await supabase
            .from('Clam_Decussatus')
            .select(`
                abbr,
                primer,
                forward,
        ...
        if (error) {
            console.error("fetchSeabassData: ERRO ao buscar dados de Clam_Decussatus:", error.message);
            return [];
        }

        allSeabassSamples = data;
        console.log("fetchSeabassData: Dados de Clam_Decussatus buscados com sucesso:", allSeabassSamples);
        return data;
    } catch (error) {
        console.error("fetchSeabassData: ERRO inesperado ao buscar dados de Clam_Decussatus:", error);
        return [];
    }
}
...
document.addEventListener('DOMContentLoaded', async () => {
    console.log("DOMContentLoaded: Página Clam_Decussatus carregada. Iniciando setup...");
    ...
    if (errorSeabass || errorBox) {
        ...
        console.error("Erro ao excluir da Clam_Decussatus:", errorSeabass);
...
document.getElementById('editBoxCountBtn').addEventListener('click', async () => {
  ...
  const { data, error } = await supabase
    .from('config')
    .select('num_boxes')
    .eq('page', 'ClamDecussatus') // ajuste o nome da página conforme sua base
    .single();
...
async function fetchBoxCount() {
  try {
    const { data, error } = await supabase
      .from('config')
      .select('num_boxes')
      .eq('page', 'ClamDecussatus') // nome da página
      .single();

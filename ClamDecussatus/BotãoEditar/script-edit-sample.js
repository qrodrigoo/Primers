...
form.addEventListener('submit', async (event) => {
  ...
  if (sampleToEdit) {
    ...
    ({ error: errorSeabass } = await supabase.from('Clam_Decussatus').insert([baseData]));
  ...
  if (errorSeabass || errorBox) {
    alert('Erro ao salvar:\n' +
      ...
      (errorBox?.message || ''));
    console.error('Erro Clam_Decussatus:', errorSeabass);
    ...
    window.location.href = '../clam-decussatus-index.html';
  }
});

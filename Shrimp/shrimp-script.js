...
async function testSupabaseConnection() {
    ...
    try {
        const { data, error } = await supabase
            .from('Shrimp') // Tente buscar de uma de suas tabelas principais
...
async function fetchBoxLocations() {
    ...
    try {
        ...
        console.log("fetchBoxLocations: Dados brutos de Shrimp_BOX recebidos:", data);
...
async function fetchSeabassData() {
    console.log("fetchSeabassData: Tentando buscar todos os dados de Shrimp...");
    try {
        const { data, error } = await supabase
            .from('Shrimp')
            .select(`
        ...
        if (error) {
            console.error("fetchSeabassData: ERRO ao buscar dados de Shrimp:", error.message);
            return [];
        }

        allSeabassSamples = data;
        console.log("fetchSeabassData: Dados de Shrimp buscados com sucesso:", allSeabassSamples);
        return data;
    } catch (error) {
        console.error("fetchSeabassData: ERRO inesperado ao buscar dados de Shrimp:", error);
        return [];
    }
}
...
function displayDetails(primerData) {
    ...
    const detailMapping = {
        ...
        'biorad-observations': 'biorad_384.observations'
    };
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (errorSeabass || errorBox) {
        ...
        console.error("Erro ao excluir da Shrimp:", errorSeabass);
        console.error("Erro ao excluir da Shrimp_BOX:", errorBox);
        return;
    }
...
document.getElementById('editBoxCountBtn').addEventListener('click', async () => {
  ...
  const { data, error } = await supabase
    .from('config')
    .select('num_boxes')
    .eq('page', 'Shrimp') // ajuste o nome da página conforme sua base
    .single();

...
async function testSupabaseConnection() {
    ...
    try {
        const { data, error } = await supabase
            .from('Seabream') // Tente buscar de uma de suas tabelas principais
...
async function fetchSeabassData() {
    console.log("fetchSeabassData: Tentando buscar todos os dados de Seabream...");
    try {
        const { data, error } = await supabase
            .from('Seabream')
            .select(`
                ...
                "head-hidney.eppenforf-real-time.slope",
                "head-hidney.eppenforf-real-time.efficiency",
                "head-hidney.eppenforf-real-time.efficiency(%)",
                "head-hidney.eppenforf-real-time.observations",

                "head-hidney.biorad-real-time.slope",
                "head-hidney.biorad-real-time.efficiency",
                "head-hidney.biorad-real-time.efficiency(%)",
                "head-hidney.biorad-real-time.observations",

                "liver.eppenforf-real-time.slope",
                "liver.eppenforf-real-time.efficiency",
                "liver.eppenforf-real-time.efficiency(%)",
                "liver.eppenforf-real-time.observations",
        ...
        if (error) {
            console.error("fetchSeabassData: ERRO ao buscar dados de Seabream:", error.message);
            return [];
        }

        allSeabassSamples = data;
        console.log("fetchSeabassData: Dados de Seabream buscados com sucesso:", allSeabassSamples);
        return data;
    } catch (error) {
        console.error("fetchSeabassData: ERRO inesperado ao buscar dados de Seabream:", error);
        return [];
    }
}
...
function displayDetails(primerData) {
    ...
    const detailMapping = {
        ...
        'eppendorf-slope': 'head-hidney.eppenforf-real-time.slope',
        'eppendorf-efficiency': 'head-hidney.eppenforf-real-time.efficiency',
        'eppendorf-efficiency-percent': 'head-hidney.eppenforf-real-time.efficiency(%)',
        'eppendorf-observations': 'head-hidney.eppenforf-real-time.observations',
        ...
        'intestine-observations': 'biorad.observations'
    };
...
document.getElementById('editBoxCountBtn').addEventListener('click', async () => {
  ...
  const { data, error } = await supabase
    .from('config')
    .select('num_boxes')
    .eq('page', 'Seabream') // ajuste o nome da página conforme sua base
    .single();
...
async function fetchBoxCount() {
  try {
    const { data, error } = await supabase
      .from('config')
      .select('num_boxes')
      .eq('page', 'Seabream') // nome da página
      .single();
